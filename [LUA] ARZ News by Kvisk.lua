...
function main()
	...
	while true do
		...
		if sampIsDialogActive() and u8:encode(sampGetDialogCaption()) == '{BFBBBA}Редактирование' then -- Есть недороботки and u8:encode(sampGetDialogCaption()) == '{BFBBBA}Редактирование'
...
function ev.onShowDialog(id, style, title, button1, button2, text)
	...
	-- sampAddChatMessage('button1: '..button1, -1)
	-- sampAddChatMessage('button2: '..button2, -1)
	-- sampAddChatMessage('text: '..text, -1)
	-- saveFile('test.txt', {text})
...
function ev.onServerMessage(color, text)
	...
	-- [Ошибка] {ffffff}У Вас нет в данный момент активных предложений, попробуйте позже.

	-- col: -1104335361, t: [Ошибка] {ffffff}У Вас нет в данный момент активных предложений, попробуйте позже.
	-- col: -1104335361, t: [Ошибка] {ffffff}У Вас нет в данный момент активных предложений, попробуйте позже.
	--sampAddChatMessage('col: '..color..', t: '..text, -1)
end
...
imgui.OnFrame(function() return rHelp[0] end,
	function(player)
		...
		imgui.Begin('Help Kvisk ##window_2', rHelp, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize)
			...
			for i=2, #autbincfg*2-1 do
				...
				if imgui.InputText(id_name..'input_S2_'..i..m, inputReplace, sizeof(inputReplace) - 1) then
					if str(inputReplace) ~= '' and str(inputReplace) ~= autbincfg[i][m] then
						autbincfg[i][m] = str(inputReplace)--; spike = true
						saveFile('autoBind.cfg', autbincfg) -- Тут временный фикс бага с последним элементом
					end
				end
				-- if not imgui.IsItemActive() and spike then
				-- 	spike = false; saveFile('autoBind.cfg', autbincfg)
				-- end
...
-- function imgui.ImVec4ToU32(a, r, g, b)
-- 	local argb = b  -- b
...
-- 	return argb
-- end
-- function imgui.U32ToImVec4(color)
...
--[[
	local nabor = {"", "q","w","e","r","t","y", "u", "i", "o", "p","a","s","d", "f", "g", "h", "j", "k", "l", "z", "x", "c", "v", "b", "n", "m",
	...
	imgui.GetScrollMaxX(void) -- макс Х
	imgui.GetScrollX(void) -- текущий Х
	imgui.GetScrollMaxY(void) -- макс Y
	imgui.GetScrollY(void) -- текущий Y
	imgui.SetScrollX(floatscroll_x) -- установить Х
	imgui.SetScrollY(floatscroll_x) -- установить Y


	-- {tagCNN} - Тег в департамент (СМИ СФ)
	-- {music} - Музыкальная заставка (<<< Музыкальная заставка радиостанции Prodigy News >>>)
	-- {server} - Сервер (Юма, Феникс и т.п.)
	-- {tag} - Тег мероприятия (прятки, реклама, математика и т.п.) --
	-- {duty} - Должность (Режисер)
	-- {city} - Город (Сан-Фиерро)
	-- {name} - Имя (Кит Флинт)
	-- {prize} - Награда (750.000) -- 
	-- {scores} - Баллы мероприятия (1, 2, 5 и т.п.) -- 
	-- {ID} - Никнейм игрока (Kit Flint) --
	-- {scoreID} - Колличество баллов конкретного игрока (1, 3, 5 и т.д.) --

]]--


-- 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

-- Сделать для смс которые приходят во вмеря эфиров отдельный сборник


-- фикс спец символов в автозамене
-- бинды теперь работают
